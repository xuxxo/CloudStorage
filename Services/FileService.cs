...
namespace FilesAPI.Services
{
    public class FileService
    {
        ...
        public UserFileDto? GetFile(long id) => _appContext.Files.FirstOrDefault(x => x.Id == id && x.IsDeleted == false);

        public UserFileDto? GetDeletedFile(long id) => _appContext.Files.FirstOrDefault(x => x.Id == id && x.IsDeleted == true);

        public List<UserFileDto> GetAllUserFiles(long id) => _appContext.Files.Where(x => x.UserId == id && x.IsDeleted == false).ToList();

        public List<UserFileDto> GetAllDeletedUserFiles(long id) => _appContext.Files.Where(x => x.UserId == id && x.IsDeleted == true).ToList();
